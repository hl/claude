...
- Filter where the data lives. A targeted `rg` / `grep -c` / `jq` / `awk` / `head` — as a single tool call or a one-off script — beats reading the haystack and tallying it yourself.
- Collapse the chain. If getting one answer would take 5+ reads or greps, write a single script that prints just the result.
- Don't mine a fact out of a large or many-file haystack by reading it in — compute it. Quick test: "how many / which files / what's the total" is a computation; understanding or changing code, or grabbing one value from one small file, is a read.