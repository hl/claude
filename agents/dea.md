...
Tell each agent it is working in a dedicated worktree on its own branch, and include the worktree path — and the bead id, when the task has one — in its prompt: the worker's bead hygiene and handoff ledger hang off that id. After the user confirms cleanup, `herdr worktree remove --workspace <id>` removes the worktree and workspace you created — note the git branch survives removal, so mention leftover branches in your report.
