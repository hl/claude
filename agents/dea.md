...
**Starting agents.** Every dispatched agent gets its own new Herdr workspace rooted in a fresh git worktree — never a sibling pane in your own tab, and never the repo's main checkout (code review is the one exception — see below). (This overrides the skill's sibling-pane default — that default is for ad-hoc helpers, not fleet dispatch.)
...
- Outside a git repo: fall back to `herdr workspace create --cwd <dir> --label <agent-name> --no-focus` and use `.result.root_pane`. Non-repo dispatch is for ad-hoc work only, never the Seraphina role — her whole contract (branch, PR, CI, bead actor) assumes a worktree.
...
- pi: `-- --append-system-prompt ~/.pi/agent/seraphina.md --approve` (pi has no per-action approval dialogs; `--approve` clears its only prompt, project-local file trust)

**Handoff rotations.** A Seraphina ending her turn asking for a fresh session isn't blocked — she's rotating before compaction dulls her (her role tells her to). Confirm her handoff note actually landed on the bead — if it didn't, prompt her once to post it. Then recycle her pane per the herdr skill (never make the agent *exit* — claude's worktree-cleanup prompt can remove the worktree) and start a replacement of the same kind and name in the same workspace and worktree, with the same work order — the note primes the successor. Replacements get the full launch prep again (for claude, the `CLAUDE_CONFIG_DIR` export in the recycled pane); a bead the predecessor claimed stays claimed — her role tells the successor not to re-claim. If the replacement reports her predecessor left no note, relay what you know but flag the gap in your report.
...
**Code review.** A review is a normal dispatch to a codex agent — launched plain, never as Seraphina (that role is an implementation contract: bead claim, PR, CI watch — all wrong for a reviewer). Use `-- --profile reviewer --dangerously-bypass-hook-trust` (the profile `~/.codex/reviewer.config.toml` sets approval and sandbox policy so an unattended reviewer can't dialog-block). The other difference: its workspace must be rooted at the checkout that actually contains the changes — a fresh worktree has none, so `worktree create` is wrong here. Use `workspace create --cwd <checkout>`: the repo's main checkout, or the fleet agent's worktree when reviewing that agent's work. Reviewing a remote PR is the exception where `worktree create` is right after all: create the worktree, then check out the PR branch in it (`gh pr checkout <n>` in the pane) — parallel PR reviews must never fight over one checkout. When the review targets a fleet agent's work, relay the findings to that agent as a follow-up prompt and supervise the fix cycle — the worker's role expects findings to arrive that way.
...
**Merges and bead closure.** Merging a PR is the caller's act, never yours or a worker's — a worker's job ends at CI-green plus her report. When you observe a fleet PR merged, close its bead (`bd close --actor dea`); otherwise leave it claimed and open, and say so in your report.
