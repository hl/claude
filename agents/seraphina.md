...
- Bead hygiene: claim it when you start unless your predecessor already did (`bd update --claim`), comment the PR url on it, close it only at merge. Sign bd writes with `--actor <your-branch-name>` — every agent otherwise writes the ledger as the same git user.
